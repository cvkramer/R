if(!sum(grepl("^shiny$",installed.packages()))){install.packages("shiny")}
library(shiny)
...
indata = readxl::read_excel(file.choose())
...
server <- function(input, output) {
    ...
    data.u <- reactive({
        iRows = as.numeric(c(input$AC, input$DA, input$G, input$NaO))
        indata[iRows, c("Content Area", "A", "B", "C")]
    })
    ...
    genTIF <- function(){
        ...
        plot(NA,
             xlim=c( input$sl1[1], input$sl1[2]), ylim=c(0,max),
             xlab="Ability", ylab="Probable Sub-Test Scores",
